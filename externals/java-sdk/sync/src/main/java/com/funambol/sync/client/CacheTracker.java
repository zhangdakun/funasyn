...
/**
 * This class implements a ChangesTracker and it is based on comparison of
 * fingerprints. This means that the class can take a snapshot of the SyncSource
 * and store it in a StringKeyValueStore (a parameter the client must provide).
 * For each item in the SyncSource its fingerprint is stored in the store. When
 * getNewItems, getUpdatedItems and getDeletedItems are invoked, they compare
 * the SyncSource current state and the last snapshot and detect changes. By
 * default MD5 is used to compute fingerprints, but the method can be redefined
 * if a client wants to use a different method.
 */
public class CacheTracker implements ChangesTracker {
	...
	/**
	 * Creates a CacheTracker. The constructor detects changes so that the
	 * method to get the changes can be used right away
	 * 
	 * @param status
	 *            is the key value store with stored data
	 */
	...
	/**
	 * This method cleans any pending change. In the cache sync source this
	 * means that the fingerprint of each item is updated to its current value.
	 * The fingerprint tables will contain exactly the same items that are
	 * currently in the Sync source.
	 */
			...
			try {
				...
				while (snapshotKeys.hasMoreElements()) {
					...
					if (status.get(newKey) == null) {
						...
						if (!oldFP.equals(newFP)) {
							if (Log.isLoggable(Log.INFO)) {
								Log.info(TAG_LOG, "New fingerprint is: "
										+ newFP);
								Log.info(TAG_LOG, "Old fingerprint is: "
										+ oldFP);
								Log.info(TAG_LOG,
										"Found an updated item with key: "
												+ newKey);
							}
				...
				while (statusKeys.hasMoreElements()) {
					...
					if (snapshot.get(oldKey) == null) {
						if (Log.isLoggable(Log.INFO)) {
							Log.info(TAG_LOG, "Found a deleted item with key: "
									+ oldKey);
						}
	 ...
	 * 
	 * @return the list of new items as an Enumeration of SyncItem
	 */
	 ...
	 * 
	 * @return the list of updated items as an Enumeration of SyncItem
	 */
	...
	/**
	 * Returns the number of deleted items that will be returned by the
	 * getDeletedItems method
	 * 
	 * @return the number of items
	 */
	 ...
	 * 
	 * @return the list of updated items as an Enumeration of strings
	 *         (SyncItem's keys)
	 */
	...
	protected boolean isSuccess(int status) {
		...
		return status == SyncSource.SUCCESS_STATUS
				|| status == SyncSource.CHUNK_SUCCESS_STATUS;
	}
		...
		try {
			...
			case SyncItem.STATE_NEW:
				try {
					...
					SyncItem deviceItem = new SyncItem(key,null,SyncItem.STATE_UPDATED,null);
					deviceItem = getItemContent(deviceItem);
					fp = computeFingerprint(deviceItem);

					if (status.get(key) != null)
						status.update(key, fp);
					else
						status.add(key, fp);
	 ...
	 * @param removed
	 *            specified if the list of items to be checked contains removed
	 *            items
	 */
	protected void filterOutOutgoingItems(Hashtable itemsToFilterOut,
			boolean removed) {
		if (null == itemsToFilterOut || 0 == itemsToFilterOut.size())
			return;
		...
		while (keys.hasMoreElements()) {
			...
			if (filteredOut) {
				if (Log.isLoggable(Log.TRACE)) {
					Log.trace(TAG_LOG, "Item with key " + key
							+ " was filtered out");
				}
	...
	/**
	 * Delegates to SyncSource all the logic for detecting items to filter out
	 */
