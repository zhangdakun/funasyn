...
public class SapiSyncStatus implements SyncReport {
	...
	private static StringKeyValueStoreFactory storeFactory = StringKeyValueStoreFactory
			.getInstance();

	public SapiSyncStatus(String sourceName) {
		this.sourceName = sourceName;
		store = storeFactory.getStringKeyValueStore(SYNC_STATUS_TABLE_PREFIX
				+ sourceName);
	}
	...
	public void setSentItemStatus(String guid, String luid, char cmd,
			int statusCode) {
		SentItemStatus status = (SentItemStatus) pendingSentItems.get(luid);
		if (status == null) {
			status = (SentItemStatus) sentItems.get(luid);
		}
		if (status == null) {
			addSentItem(guid, luid, cmd, statusCode);
		} else {
			status.setStatus(statusCode);
			status.setGuid(guid);
			// In order for the item to be recorded again at the next save, it
			// must be moved into the pending
			sentItems.remove(luid);
			pendingSentItems.put(luid, status);
		}
	}
	...
	public int getSentItemsCount() {
		...
		while (pendingSentItemsEnum.hasMoreElements()) {
			SentItemStatus s = (SentItemStatus) pendingSentItemsEnum
					.nextElement();
			if (s.getStatus() != SyncSource.INTERRUPTED_STATUS) {
				count++;
			}
	...
	public void receivedItemStatus(String key, int status) {
		...
		if (itemStatus == null) {
			Log.error(TAG_LOG,
					"Setting the status for an item which was not sent " + key);
		} else {
	 ...
	 * 
	 * @param guid
	 *            the server id for the item
	 * @param luid
	 *            the client id for the item
	 * @param cmd
	 *            the command the server sent the item into
	 * @param status
	 *            the client status for this command
	 */
	public void addReceivedItem(String guid, String luid, char cmd,
			int statusCode) {
		...
		if (pendingReceivedItems.containsKey(luid)) {
			ReceivedItemStatus oldStatus = (ReceivedItemStatus) pendingReceivedItems
					.get(luid);
	...
	public int getReceivedItemsCount() {
		// We shall discard the interrupted items as they cannot be considered
		// as
		// completed
		int count = 0;
		Enumeration receivedItemsEnum = receivedItems.elements();
		while (receivedItemsEnum.hasMoreElements()) {
			ReceivedItemStatus s = (ReceivedItemStatus) receivedItemsEnum
					.nextElement();
			if (s.getStatus() != SyncSource.INTERRUPTED_STATUS) {
				count++;
			}
		...
		while (pendingReceivedItemsEnum.hasMoreElements()) {
			ReceivedItemStatus s = (ReceivedItemStatus) pendingReceivedItemsEnum
					.nextElement();
			if (s.getStatus() != SyncSource.INTERRUPTED_STATUS) {
				count++;
			}
	...
	public void addMappingSent(String luid) {
		ReceivedItemStatus status = (ReceivedItemStatus) receivedItems
				.get(luid);
		if (status == null) {
	...
	public String getReceivedItemLuid(String guid) {
		...
		while (keys.hasMoreElements()) {
			String luid = (String) keys.nextElement();
			ReceivedItemStatus status = (ReceivedItemStatus) pendingReceivedItems
					.get(luid);
			String g = status.getGuid();
			if (guid.equals(g)) {
				return luid;
	...
	public void setReceivedItemStatus(String guid, String luid, char cmd, int s) {
		ReceivedItemStatus status = (ReceivedItemStatus) pendingReceivedItems
				.get(luid);
		if (status == null) {
			status = (ReceivedItemStatus) receivedItems.get(luid);
		}
		if (status != null) {
			status.setStatus(s);
		} else {
			addReceivedItem(guid, luid, cmd, s);
		}
	}
		...
		try {
			...
			while (keys.hasMoreElements()) {
				...
				if (REQUESTED_SYNC_MODE_KEY.equals(key)) {
					...
					ReceivedItemStatus status = new ReceivedItemStatus(guid,
							cmd);
			...
			initialReceivedAddNumber = getReceivedAddNumber();
			initialReceivedUpdNumber = getReceivedReplaceNumber();
			initialReceivedDelNumber = getReceivedDeleteNumber();

			initialSentAddNumber = getSentAddNumber();
			initialSentUpdNumber = getSentReplaceNumber();
			initialSentDelNumber = getSentDeleteNumber();
		} catch (Exception e) {
	...
	public void save() throws IOException {
		...
		while (keys.hasMoreElements()) {
			...
			// Check if this is a real new item or an update
			StringBuffer kb = new StringBuffer();
			kb.append(SENT_ITEM_KEY).append(key);
			String k = kb.toString();
			try {
				if (store.contains(k)) {

					store.update(k, v.toString());

				} else {
					store.add(k, v.toString());
				}
		...
		while (keys.hasMoreElements()) {
			...
			StringBuffer kb = new StringBuffer();
			kb.append(RECEIVED_ITEM_KEY).append(key);
			String k = kb.toString();
			try {
				if (store.contains(k)) {
					store.update(k, v.toString());
		...
		try {
			if (oldRequestedSyncMode != requestedSyncMode) {
				if (oldRequestedSyncMode == -1) {
					...
					store.update(REQUESTED_SYNC_MODE_KEY, ""
							+ requestedSyncMode);
				}
				oldRequestedSyncMode = requestedSyncMode;
			}
			...
			if (oldLastSyncStartTime != lastSyncStartTime) {
				if (oldLastSyncStartTime == 0) {
					...
					store.update(LAST_SYNC_START_TIME_KEY, ""
							+ lastSyncStartTime);
				}
	...
	public int getNumberOfReceivedItemsWithSyncStatus(int syncStatus) {
		return getNumberOfItemsWithSyncStatus(receivedItems, syncStatus)
				+ getNumberOfItemsWithSyncStatus(pendingReceivedItems,
						syncStatus);
	}
	...
	public String toString() {
		...
		res.append("| Local URI: ").append(locUri).append(" - Remote URI:")
				.append(remoteUri).append("\n");
		...
		res.append("| Replace: ").append(getReceivedReplaceNumber())
				.append("\n");
		res.append("| Delete: ").append(getReceivedDeleteNumber()).append("\n");
		res.append("| Resumed: ").append(getReceivedResumedNumber())
				.append("\n");
		res.append("| Total errors: ")
				.append(getNumberOfReceivedItemsWithError()).append("\n");
		res.append("|-----------------------------------------------------------------\n");
		...
		res.append("| Total errors: ").append(getNumberOfSentItemsWithError())
				.append("\n");
		res.append("|-----------------------------------------------------------------\n");
		res.append("| Global sync status: ").append(getStatusCode())
				.append("\n");
		res.append("|-----------------------------------------------------------------\n");
