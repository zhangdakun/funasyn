...
public final class HttpTransportAgent implements TransportAgent {
    ...
    // Users can set their own headers by proving an hashtable
    ...
    public HttpTransportAgent(String requestURL, boolean compress,
        ...
        this(requestURL, null, null, compress, forceCookies, null);
    }
    ...
    public HttpTransportAgent(String requestURL, final String userAgent,
        ...
        this(requestURL, userAgent, null, compress, forceCookies, null);
    }

    /**
     * Initialize a new HttpTransportAgent with a URL, user agent, charset,
     * compression and cookies force.
     *
    ...
    public HttpTransportAgent(String requestURL,
            ...
            boolean compress,
            boolean forceCookies) {
        this(requestURL, userAgent, charset, compress, forceCookies, null);
    }
    

    /**
     * Initialize a new HttpTransportAgent with a URL, user agent, charset,
     * compression, cookies force and proxy to use.
     *
     * @param requestURL must be non-null
     * @param userAgent a string to be used as userAgent.
     * @param charset a valid charset, the device charset is used by default.
     ...
     * @param forceCookies if true sets http headers to force the use of cookies 
     * instead of Url Rewriting to manage the current http session.
     * @param proxyConfig proxy configuration to use, <code>null</code> for no proxy
     *
     */
    public HttpTransportAgent(String requestURL,
            final String userAgent,
...
//        }
    ...
    private void waitToConnect() {
        try {
            ...
            Log.error(TAG_LOG, "Connection timer failed", ex);
        }
    }
                       ...
                       String tempCookie = c.getHeaderField("set-cookie");
                        ...
                        if ((len == -1) && (responseDate == null) && (responseContentType == null)) {
                            ...
                            if (enableCompression) {
                                if (Log.isLoggable(Log.DEBUG)) {
                                    Log.debug(TAG_LOG, "##### WAP SIMULATION with compression error#######");
                                }
                                throw new CodedException(CodedException.ERR_READING_COMPRESSED_DATA,
                                                         "Error reading compressed response");
                            }
...
//            }
    ...
    public void setCustomHeaders(Hashtable customHeaders) {
        this.customHeaders = customHeaders;
    }

    private void setBackwardCompatibilityHeaders(HttpConnectionAdapter c) throws IOException {
    ...
    private void setCustomHeaders(Hashtable headers, HttpConnectionAdapter c) throws IOException {
    ...
    private void setConfig(HttpConnectionAdapter c, int length) throws IOException {
        ...
        // The request method is always a POST
        c.setRequestMethod(HttpConnectionAdapter.POST);

        // For backward compatibility we keep adding headers that we used to add
        // even though it is unclear if they are needed
        setBackwardCompatibilityHeaders(c);
...
//        X-ECLIENT-VERSION
        ...
        if (enableCompression) {
            ...
            if (Log.isLoggable(Log.DEBUG)) {
                Log.debug(TAG_LOG, "Encoding Response Required from Client: " + COMPRESSION_TYPE_GZIP);
            }
        }
    ...
    private void clearRequestStream() {
        if (os != null) {
            try {
                ...
                Log.error(TAG_LOG, "Can't close output stream.", ioe);
            }
        }
    }
    ...
    private void clearResponseStream() {
        if (is != null) {
            try {
                ...
                Log.error(TAG_LOG, "Can't close input stream.", ioe);
            }
        }
    }
    ...
    protected class ConnectionTimer extends TimerTask {
         ...
         **/
        public void run() {
            Log.error(TAG_LOG, "An IO operation did not complete before maximum allowed time.");
