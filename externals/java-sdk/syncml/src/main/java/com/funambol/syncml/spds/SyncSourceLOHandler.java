...
class SyncSourceLOHandler {
    ...
    throws SyncException {
        ...
        if (resume && !deletesResumed) {
            ...
            while(sentItemKeysForResume.hasMoreElements()) {
                ...
                if (source instanceof ResumableSource) {
                    ResumableSource rss = (ResumableSource)source;
    ...
    private Chunk getNextItemWithResumeFilter(SyncStatus syncStatus) {
        ...
        // If we get here, the source must be resumable
        ResumableSource rss = (ResumableSource)source;
        ...
        do {
            ...
            if (SyncMLStatus.isSuccess(status)) {
                ...
                // sync
                if (rss.hasChangedSinceLastSync(chunk.getKey(), syncStatus.getLastSyncStartTime())) {
                    if (Log.isLoggable(Log.INFO)) {
