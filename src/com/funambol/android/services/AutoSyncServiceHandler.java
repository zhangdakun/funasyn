...
public class AutoSyncServiceHandler {
    ...
    public void programScheduledSync() {
        ...
        serviceIntent.setAction("com.funambol.android.services.AutoSyncService");
    ...
    public void startSync(String syncMode, Vector sources) {
        startSync(syncMode, sources, 0);
    }

    public void startSync(String syncMode, Vector sources, int delay) {
        ...
        for(int i=0;i<sources.size();++i) {
            AppSyncSource s = (AppSyncSource)sources.elementAt(i);
            sourcesId[i] = s.getId();
        }
...
//        context.s
    ...
    public void startMonitoringDirectory(String directory, int sourceId, String extensions[]) {
        Intent serviceIntent = new Intent();
        serviceIntent.setAction("com.funambol.android.services.AutoSyncService");
        serviceIntent.putExtra(AutoSyncService.OPERATION, AutoSyncService.START_MONITORING_DIRECTORY);
        serviceIntent.putExtra(AutoSyncService.DIRECTORY, directory);
        serviceIntent.putExtra(AutoSyncService.SOURCE_ID, sourceId);
        serviceIntent.putExtra(AutoSyncService.EXTENSIONS, extensions);
        context.startService(serviceIntent);
    }
