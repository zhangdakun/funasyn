...
//lieb disable for debug show service run at notifcation bar
     ...
     * active and enabled.
     */
    private AndroidFileObserver startMonitoringDirectory(String baseDirectory,
            int sourceId, String extensions[]) {
        AndroidFileObserver fileObserver = null;
        if(!StringUtil.isNullOrEmpty(baseDirectory)) {
            fileObserver = (AndroidFileObserver)fileObservers.get(baseDirectory);
            AndroidAppSyncSource appSource =
                    (AndroidAppSyncSource)appSyncSourceManager.getSource(sourceId);
            if (fileObserver == null) {
                fileObserver = new AndroidFileObserver(appSource, baseDirectory,
                        extensions, context, configuration);
    ...
    private void programRepeatingSync(int minutes) {
        ...
        scheduledSyncIntent = PendingIntent.getBroadcast(context, 0, i,
                PendingIntent.FLAG_UPDATE_CURRENT);
        
        am = (AlarmManager) context.getSystemService(Context.ALARM_SERVICE);

        am.setRepeating(AlarmManager.RTC_WAKEUP, 
                System.currentTimeMillis() + interval, interval, scheduledSyncIntent);
    }
